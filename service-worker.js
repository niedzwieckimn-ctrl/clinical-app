const CACHE='clinical-spa-secure-v6';
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(fetch(event.request).then(response=>{const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(event.request,copy));return response}).catch(()=>caches.match(event.request,{ignoreSearch:true}).then(hit=>hit||(event.request.mode==='navigate'?caches.match('/admin/index.html'):Response.error()))));
});
